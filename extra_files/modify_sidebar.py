...
def main():
    ...
    start_str = '                            <div className="instadeed-solo-header mb-4'
    end_str = '                            </div>\n                        </div>\n\n                        {/* Scrollable Form Area */}'
    
    start_idx = content.find(start_str)
    if start_idx == -1:
        print("Error: Could not find start of target block")
        return
        
    end_idx = content.find(end_str)
    if end_idx == -1:
        print("Error: Could not find end of target block")
        return

    # Adjust end_idx to include the closing div of the doc grid but NOT the closing div of the header
    # The end_str is: '                            </div>\n                        </div>\n\n                        {/* Scrollable Form Area */}'
    # The first '                            </div>' closes the instadeed-doc-grid.
    # The second '                        </div>' closes the sticky header div.
    # We want to keep the second '                        </div>' and the scrollable area outside the conditional wrap.
    # So we replace from start_idx up to the end of the instadeed-doc-grid (just before the second </div>).
    
    grid_close_str = '                            </div>\n                        </div>'
    grid_close_idx = content.find(grid_close_str, start_idx, end_idx + 100)
    if grid_close_idx == -1:
        print("Error: Could not find grid closing div")
        return
        
    # The target block of code to be replaced is:
    # content[start_idx:grid_close_idx + len('                            </div>')]
    target_block = content[start_idx:grid_close_idx + len('                            </div>')]
    
    # Let's verify that the target block contains the expected selectors
    if 'instadeed-solo-header' not in target_block or 'instadeed-authority-selector' not in target_block or 'instadeed-doc-grid' not in target_block:
        print("Error: Target block does not contain the expected selectors")
        return

    replacement_block = """                            {activeTab === 'HOME' && (
                                <>
    ...
    # Replace content
    new_content = content[:start_idx] + replacement_block + content[grid_close_idx + len('                            </div>'):]
    
    with open('test_script.jsx', 'w', encoding='utf-8') as f:
        f.write(new_content)
        
    print("test_script.jsx successfully modified!")
